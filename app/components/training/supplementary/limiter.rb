module Training
  module Supplementary
    class Limiter
      ...
      def all_prepaid
        @course.sign_ups.includes([:emails, :payment, :user]).where(payments: { state: :prepaid, refunded_at: nil }).or(@course.sign_ups.includes([:emails, :payment, :user]).where(payments: { cash: true })).order(:created_at)
      end
