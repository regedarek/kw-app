...
describe Membership::PayFee do
  ...
  describe '.pay' do
    ...
    it 'no payments, pay for next' do
      ...
      Membership::PayFee.pay(kw_id: 1111, form: form)
      expect(Db::MembershipFee.count).to eq(1)
      expect(Db::MembershipFee.find_by(kw_id: 1111, year: 2018).cost).to eq(150)
    end
    ...
    it 'last year payment, pay for next' do
      Db::MembershipFee.create!(kw_id: 1111, year: 2016)
      form = Membership::FeeForm.new(year: 2018)
      expect(Db::MembershipFee.count).to eq(1)
      Membership::PayFee.pay(kw_id: 1111, form: form)
      expect(Db::MembershipFee.count).to eq(2)
      expect(Db::MembershipFee.find_by(kw_id: 1111).cost).to eq(150)
    end

    it 'gap in payments, pay for current' do
      Db::MembershipFee.create!(kw_id: 1111, year: 2014)
      form = Membership::FeeForm.new(year: 2017)
      expect(Db::MembershipFee.count).to eq(1)
      Membership::PayFee.pay(kw_id: 1111, form: form)
      expect(Db::MembershipFee.count).to eq(2)
